...
Estado actual: **el spine determinístico offline ya corre**, pero todavía no existe un tick de entrenamiento real. `bash lab/tick.sh --dry-run` ejecuta generación fake con cap de gasto, curación, evaluación y gate S4 fail-closed; el stage de entrenamiento sigue marcado `[STUB]` y el dry-run nunca promueve pesos. La habilitación del primer tick real depende de cerrar [`recipes/00-bootstrap.md`](recipes/00-bootstrap.md), fijar teacher/licencias y provisionar un runner spot reproducible. La GPU local sirve para toys, guard, cuantización y eval; no es requisito comprar hardware antes de medir gasto real.
